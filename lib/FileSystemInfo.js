...
class FileSystemInfo {
	...
	/**
	 * @param {Snapshot} snapshot a snpashot
	 * @param {Iterable<string>} files all files
	 * @param {Iterable<string>} directories all directories
	 * @param {Iterable<string>} missing all missing files or directories
	 * @returns {boolean} true, when the snapshot includes all items listed
	 */
	snapshotIncludes(snapshot, files, directories, missing) {
		const managedItems = new Set();
		...
		if (files) {
			files: for (const path of files) {
				...
				if (snapshot.fileTimestamps && snapshot.fileTimestamps.has(path))
					continue;
				if (snapshot.fileHashes && snapshot.fileHashes.has(path)) continue;
				return false;
			}
		}
		...
		if (directories) {
			directories: for (const path of directories) {
				...
				if (snapshot.contextTimestamps && snapshot.contextTimestamps.has(path))
					continue;
				if (snapshot.contextHashes && snapshot.contextHashes.has(path))
					continue;
				return false;
			}
		}
		...
		if (missing) {
			missing: for (const path of missing) {
				...
				if (snapshot.missingTimestamps && snapshot.missingTimestamps.has(path))
					continue;
				return false;
			}
		}
		...
		for (const path of managedItems) {
			if (snapshot.managedItemInfo && snapshot.managedItemInfo.has(path))
				continue;
			return false;
		}
